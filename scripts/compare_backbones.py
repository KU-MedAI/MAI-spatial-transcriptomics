"""Compare image backbones on the same train/validation split."""
...
def parse_args():
    ...
    parser.add_argument("--out-dir", default="runs/backbone_comparison")
    parser.add_argument(
        "--backbones",
        default="efficientnet_b0,resnet18,densenet121,mobilenet_v2,efficientnet_b7",
    )
...
def main() -> None:
    ...
    rows = []
    for backbone in [item.strip() for item in args.backbones.split(",") if item.strip()]:
        print(f"\n=== Backbone: {backbone} ===")
        _, valid_loss, scores = fit_single_split(
            ...
            config=config,
            device=device,
            checkpoint_path=out_dir / f"best_{backbone}.pth",
        )
        rows.append(summarize_score_row({"backbone": backbone}, valid_loss, scores))

    results = pd.DataFrame(rows).sort_values(["MG", "HVG", "HEG"], ascending=False)
    results.to_csv(out_dir / "backbone_comparison.csv", index=False)
    print(results)
    print(f"Saved: {out_dir / 'backbone_comparison.csv'}")
